...
The picker offers the vaults that matter to Sky, and that list is generated, not typed. Every vault the API indexes on the chains that have a Prime rate-limit contract (about 4,100 on six chains) is checked on-chain: does any Prime rate-limit contract hold a deposit limit with a non-zero maximum for it? A vault is listed if a Prime can allocate to it that way, if a Prime holds a position in it, or if a cited spell is about to onboard it. Skybase's own vaults are listed as well, for reference rather than because a Prime allocates to them: they are the vaults Sky Money's address owns or curates in Morpho's curator registry, kept only if Morpho lists them in its app, and shown as their own group ordered by size. Ownership, registry constants and Atlas mentions are recorded but never list a vault on their own, so superseded and test deployments stay out. A limit zeroed by a later spell is an offboarding. The list is rebuilt by the update described below, never edited by hand. The one typed input is the pending-spell entries in `config/vault-overrides.json`, each citing the forum proposal; the update reports an entry as executed once the chain shows the limit, and it is then deleted. `docs/VAULTS.md` is the readable version.
