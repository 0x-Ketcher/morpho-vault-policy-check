...
| Morpho curator registry (API `curators`); on a line that names several Safes ("held by the Spark Foundation multisig at 0x…, together with a Soter Labs multisig at 0x…") each address gets the holder phrase closest before it, or the holder phrase after it ("… is controlled by …"), or the line's "Role: Entity," prefix | curator name to addresses per chain: Steakhouse Financial, Sentora, SparkDAO, Sky Money, and others | `scripts/sync-labels.ts`; names that are Primes are mapped through `policy.primes.curatorRegistryAliases` |
...
Definition: a Prime rate-limit contract holds a `LIMIT_4626_DEPOSIT` key for the vault with a non-zero maximum, so a Prime agent can allocate to it (a key zeroed by a later spell is an offboarded vault and does not count); or a Prime agent holds shares in it; or a scheduled spell is about to onboard it, cited in `config/vault-overrides.json`; or Sky Money, a verified entry in Morpho's curator registry, owns or curates it and Morpho lists it in its app (the API's `listed` flag; an abandoned first deployment that Sky Money curates but Morpho never listed is left out). Governance ownership, registry constants and Atlas mentions are recorded as sources on listed vaults but do not list a vault on their own. The universe scanned is every vault the Morpho API indexes on the accepted chains, about 3,800 on 2026-09-09; the rate-limit question is one Multicall read per vault per Prime contract, a few seconds per chain on publicnode. RateLimits, ALM proxy and governance addresses come from the registries. Rebuilt by `scripts/sync-vaults.ts`; rendered as `docs/VAULTS.md`. The only hand-maintained input is `config/vault-overrides.json`: vaults a scheduled spell will onboard, each citing the proposal, marked pending until the chain shows the rate limit.

Cross-check (`scripts/verify-rate-limits.ts`, decision logic in `src/core/rate-limits.ts` with unit tests): the list asks the contracts about vaults the API knows, so a vault the API does not index could hide. The cross-check starts from the contracts instead. Every `RateLimitDataSet` event a Prime rate-limit contract ever emitted gives the complete set of keys ever configured (Etherscan logs API on Ethereum, Arbitrum and Unichain; Blockscout's logs API on Base and Optimism, where Etherscan's free plan serves no logs; the official Robinhood node on Robinhood). Each key is read live and matched against keys computed for every Morpho vault on all fourteen API chains, every registry address, and every token contract and counterparty in the Prime ALM proxies' token-transfer history (explorer `tokentx`), under every `LIMIT_*` prefix found in the Prime controllers' verified source code (Etherscan `getsourcecode`) and in each encoding those sources use: the prefix alone, prefix plus address, prefix plus CCTP domain, asset plus destination, and address plus LayerZero or Centrifuge id. Separately, every address in the creation events of every Morpho vault factory the API knows is confirmed as a vault by the factory contract itself (`isMetaMorpho` / `isVaultV2`); a confirmed vault the API does not index has its deposit key checked against every live key. A live `LIMIT_4626_DEPOSIT` key for a Morpho vault missing from the list fails the run (exit 1); a contract whose logs no explorer would serve is named in the result line and the run exits 10. Blockscout's keyless API allows about ten calls per window per host, so its chains skip the optional token-transfer step. Prime rate-limit contracts on Avalanche, Plume and X Layer are out of scope because Morpho is not deployed there.

## Keeping the tables fresh

`scripts/sync-labels.ts` asks GitHub for the head commit of each source repo, re-reads the files, and writes `labels/*.json` only when something changed; the run timestamp alone is not a change, so a quiet day leaves the repository untouched. It exits 0 (nothing), 10 (only the pinned commits moved) or 20 (a label changed). `npm run update` runs it together with the vault-list rebuild, the Safe re-read and the docs, by hand, following `docs/UPDATE.md`; nothing runs on a schedule. The page compares the pinned commits with the current heads at load time and shows a banner when a source has moved, which is the signal that an update is due.

Change rate measured on 2026-09-04: the Atlas had 33 commits on `main` since 2026-08-05, but only 2 address lines changed in the Spark article and 2 in the Grove article over three weeks; the registries had 6 (Spark) and 2 (Grove) commits. Expect the update to find a label change one to three times a month.
