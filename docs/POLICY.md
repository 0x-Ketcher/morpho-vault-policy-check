...
|---|---|---|---|---|
| C1 | Chain | Ethereum (1), Base (8453), Robinhood Chain (4663) | FAIL | Policy section 'Accepted chains': Ethereum, Base, Robinhood (higher CRR for non-mainnet chain risk). Others: risk review + separate request. |
| C2 | Loan asset | address allow-list per chain (below) | FAIL | Policy section 'Loan assets': per Atlas cash stablecoins USDC, USDT, pyUSD; also historically approved for Morpho: RLUSD, USDG. Any other loan asset: risk review + separate request. USDS is accepted in this tool although the policy does not list it; BA Labs confirmation pending. |
| C3 | Collateral, LLTV and IRM | accepted collateral within max LLTV, and the accepted interest rate model, for every market with an allocation or a cap | not accepted with allocation FAIL; cap-only WARN; LLTV above max FAIL/WARN; symbol match but unknown address WARN; wrong IRM FAIL/WARN | Policy section 'Accepted Collateral and Market Parameters' (LLTV = maximum, lower tiers acceptable): ETH 86%, cbBTC 86%, stETH 86%, WBTC 86%, sUSDS 96.5%. Policy table 'Accepted Collateral and Market Parameters', IRM column: 'Morpho Adaptive Curve IRM' on every accepted row. Footnote: the IRM criterion applies to Morpho Blue v1 markets only; Markets V2 (Midnight) set rates through makers and takers and have no IRM. |
...
| C7 | Timelocks | per-function rules (below) | FAIL | Policy section 'Accepted timelocks per function (v2)' and 'Adapter timelocks (added Aug 31)'. Minimums; abdication accepted only where the policy says '7d/Abdicated', and it fails any other row. Rows with 'exactDays: 0' must carry no delay at all; a delay on those rows is FAIL. |
...
| Ethereum | USDS | 0xdC035D45d973E3EC169d2276DDab16f1e407384F | Spark registry Ethereum.sol USDS (also Grove, Keel and Skybase registries); Morpho API asset of the Spark USDS Vault and the sky.money USDS vaults |
...
| Base | USDS | 0x820C137fa70C8691f0e44Dc420a5e53c168921Dc | Spark and Grove registries Base.sol USDS |
...
| vault | `setIsAllocator(address,bool)` | Add/Remove allocator | no delay allowed | no | Policy row '0 days' (changed from 3 days on 2026-09-10, per the doc's comment): the same key grants and revokes allocator permission; removing the delay removes the risk from a compromised allocator. |
...
| vault | `setForceDeallocatePenalty(address,uint256)` | Set force deallocate penalty | no delay allowed | no | Policy row '0 days' (changed from 7 days on 2026-09-14, per the doc's comment): following Morpho's recommendation, so a penalty can be introduced quickly if needed. |
...
| vault | `setSendAssetsGate(address)` | Set send assets gate | at least 7d | no | The policy lists this gate as '7 days' only, without '/Abdicated', unlike the other three gates: a live timelock of at least 7 days is required and abdication fails the row. |
