...
| C2 | Loan asset | address allow-list per chain (below) | FAIL | Policy section 'Loan assets': per Atlas cash stablecoins USDC, USDT, pyUSD; also historically approved for Morpho: RLUSD, USDG. Any other loan asset: risk review + separate request. USDS added 2026-09-10 by Sky GovOps decision: the criteria document names neither its inclusion nor its exclusion, no public decision excludes it, and it is the loan asset of the Spark USDS Morpho vault (curated under BA Labs review since 2025), of the sky.money Vault V2 deployments, and of the Atlas Integration Boost for USDS in Morpho (A.6.1.1.4.2.5.2.2.3). USDS is accepted on every accepted chain; on Robinhood no address is published yet. Confirmation requested from BA Labs; DAI is not added. |
...
| Robinhood Chain | USDS | undefined | Accepted on every accepted chain by the Sky GovOps decision of 2026-09-10. No USDS deployment on Robinhood Chain is published by a Prime registry, the Morpho API or the Atlas as of 2026-09-10, so there is no address to match: a USDS-named token here grades WARN until a registry publishes the address. |
