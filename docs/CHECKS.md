...
| C5 | Curator | `curator()`; Safe reads, one level of nesting | `curator.address` / `state.curator`; Safe service | Morpho curator registry (external curators), registry multisigs, Atlas role lines, OEA entity names | 2/2 of external curator + OEA = PASS, where the OEA half is labeled directly or a public source names the pair ("Curator: Soter Labs and Sentora, 2 of 2" in the Atlas); external + a co-signer nothing names or only signer overlap places = WARN; Prime/OEA self-curated = WARN; external only = FAIL; unattributable = FAIL |
...
| C9 | Allocators | each API-listed allocator confirmed with `isAllocator(addr)` | `allocators { allocator { address } }` / `allocators` | registry `almProxy`, curator registry | INFO; the Morpho Public Allocator contract, when its address is cited in the policy, is shown as acceptable per the criteria (comment of 2026-09-10) |
