...
| C7 | Sentinel / Guardian | V2: each API-listed sentinel confirmed with `isSentinel(addr)`; v1.1: `guardian()`; Safe reads; leaf signer sets compared with the curator Safe's | `sentinels { sentinel { address } }` / `state.guardian`; Safe service | same as C6, plus the Safe-owners table for signer overlap | every seat is classified. Mandatory: one OEA Safe with signers disjoint from the curator's (directly labeled = PASS; by signer overlap or unlabeled OEA-shaped = WARN; none = FAIL). Violation: the curator address, a signer of the curator Safe, the external curator, or a Safe sharing signers with the curator Safe holds a seat = FAIL. Extras: Prime-owned = allowed; an address no public source attributes = WARN as a possible monitoring solution. Card = worst |
...
The mandatory OEA seat is judged on the best candidate, but every other seat is judged too. A curator-side sentinel is a violation on its own, whatever else is in place: the seat exists to check the curator, so the curator, its signers and the external curator may not hold it. Extra seats must be Prime-owned or a monitoring solution; since no public source labels monitoring solutions today, such a seat shows as WARN with the reason until a label exists.
