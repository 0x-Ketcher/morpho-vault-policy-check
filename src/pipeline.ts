...
export function makeReader(deps: Deps, chainId: number): OnchainReader | null {
  ...
  if (deps.etherscan && deps.etherscan.chains.includes(chainId)) endpoints.push({ label: `etherscan${deps.etherscan.apiKey ? "" : " (via server proxy)"}`, transport: etherscanTransport({ chainId, base: deps.etherscan.base, apiKey: deps.etherscan.apiKey }), maxCalldata: 2600 });
